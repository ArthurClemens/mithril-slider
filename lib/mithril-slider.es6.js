...
slider.controller = (opts = {}) => {
    ...
    const getPageEl = (el, idx) => el.querySelectorAll(pageSelector)[idx];

    const setTransitionStyle = (el, value) => {
        const style = el.style;
        const attrs = isVertical ? '0, ' + value + 'px, 0' : value + 'px, 0, 0';
        const transform = 'translate3d(' + attrs + ')';
    ...
    const updateContentSize = (el) => {
        const page = el.querySelector(pageSelector);
